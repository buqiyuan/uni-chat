...
export default {
  ...
  computed: {
    ...
    drawerStyle() {
      ...
      const positionCss = {
        [transformWithDirection('top')]: 0,
        [transformWithDirection('bottom')]: 0,
      }
      return {
        ...positionCss,
        zIndex,
        ...
        transform: `${transformWithDirection('translateX')}(${
          (drawerPos || 0) && `${$reverse ? '-' : ''}${drawerPos}px`
        }) translateZ(0)`,
      }
    },
    contentStyle() {
  ...
  mounted() {
    ...
    const initDrag = function (event) {
      ...
      document.addEventListener(
        handledEvents.up,
        removeDrag,
        supportsTouch && supportsPassive ? { passive: true } : false
      )
    ...
    const drag = function (e) {
      ...
      speed =
        ([1, -1][+$reverse] * (metric[transformWithDirection('nowX')] - metric[transformWithDirection('lastX')])) /
        (t2 - t1)
      let pos =
        startPos +
        [1, -1][+$reverse] * (metric[transformWithDirection('nowX')] - metric[transformWithDirection('startX')])
    ...
    ;['transitionend', 'webkitTransitionEnd', 'msTransitionEnd', 'otransitionend', 'oTransitionEnd'].forEach(
      (eventName) => {

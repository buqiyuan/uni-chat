...
const actions: ActionTree<ChatState, RootState> = {
  ...
  connectSocket({ commit, state, dispatch, rootState }) {
    return new Promise((resolve) => {
      ...
      console.log(`ws://${process.env.VUE_APP_API_URL!.split('http://')[1]}`, 'ws')
      // token校验,失败则要求重新登录
      ...
      socket.on('friendMessage', async (res: ServerRes) => {
        ...
        if (!res.code) {
          if (res.data.friendId === user.userId || res.data.userId === user.userId) {
            ...
            let deletedChat = (await minCache.get(`${user.userId}-deletedChatId`)) as string[]
            if (deletedChat) {
              ...
              await minCache.set(`${user.userId}-deletedChatId`, deletedChat)
            }
            const { activeRoom } = state
      ...
      socket.on('chatData', async (res: ServerRes) => {
        if (res.code) {
          return Vue.prototype.$message.error(res.msg)
        }
        console.log(res)
        await dispatch('handleChatData', res.data)
